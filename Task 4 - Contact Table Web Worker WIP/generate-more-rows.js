// Script to generate rows 251-450
...
for (let i = 251; i <= 450; i++) {
  const firstName = firstNames[(i - 251) % firstNames.length];
  const lastName = lastNames[Math.floor((i - 251) / 4) % lastNames.length];
  const name = `${firstName} ${lastName}`;
  const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}@${domains[(i - 251) % domains.length]}`;
  const phone = `555${String(Math.floor(1000000 + (i * 12345) % 9000000)).substring(0, 7)}`;
  const website = `https://${domains[(i - 251) % domains.length]}`;
  const company = `${companies[(i - 251) % companies.length]} ${companySuffixes[(i - 251) % companySuffixes.length]}`;
  const streetNum = 100 + ((i - 251) * 37) % 900;
  const street = streets[(i - 251) % streets.length];
  const city = cities[(i - 251) % cities.length];
  const state = states[(i - 251) % states.length];
